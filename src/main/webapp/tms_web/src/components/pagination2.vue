...
export default {
  ...
  methods: {
      ...
      this.getData({
        ...
        success:(data)=>{
          ...
          if(data.code){
            ...
            var user=sStorage.getItem('userName');
            if(user=='appuser'){
              this.getSureData('appuser',data.rows);
            }else if(user=='jiangsu'){
              this.getSureData('jiangsu',data.rows);
            }else{
    ...
    getSureData(userName,data){
      var arr=[];
      var length=data.length;
      for(var i=0; i<length; i++){
          if(data[i].appContact==userName){
              arr.push(data[i]);
          }
      }
      if(!arr.length){
